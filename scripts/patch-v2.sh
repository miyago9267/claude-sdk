...
#
# 用法：
#   bash scripts/patch-v2.sh          # 正常 patch
#   bash scripts/patch-v2.sh --check   # 只檢查不修改
#   bash scripts/patch-v2.sh --revert   # 從 .bak 還原
# ─────────────────────────────────────────────────────────────
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SDK_MJS="$SCRIPT_DIR/../node_modules/@anthropic-ai/claude-agent-sdk/sdk.mjs"
BACKUP="$SDK_MJS.bak"
